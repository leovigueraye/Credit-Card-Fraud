...
# Title
st.title('Web Based Credit Card Fraud Detection System')
...
if st.sidebar.checkbox('Manual transaction verification'):
    ...
    st.write("Select the row index to check if the transaction is legitimate or fraudulent:")
    ...
    if st.button("Predict"):
        # Get the features for the selected row index
        features = df.drop(columns="Class", axis=1).iloc[index].values
        # Make prediction
